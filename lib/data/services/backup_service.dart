...
class BackupService extends ChangeNotifier {
  ...
  Future<bool> backupImages(List<String> imagePaths, {String projectName = 'Mặc định'}) async {
    ...
    try {
      ...
      final existingFilesList = await _driveApi!.files.list(q: existingFilesQuery, $fields: 'files(id, name)');
      final existingFilesMap = {for (var f in existingFilesList.files ?? []) f.name: f.id};
      ...
      for (final imagePath in imagePaths) {
        ...
        final existingFileId = existingFilesMap[fileName];

        if (existingFileId != null) {
          // Update existing file
          debugPrint('🔄 Drive: Updating existing file $fileName (ID: $existingFileId)');
          final fileMetadata = drive.File();
          ...
          debugPrint('✅ Drive: Update successful for $fileName');
        } else {
          // Create new file
          debugPrint('📤 Drive: Creating new file $fileName');
          final fileMetadata = drive.File()
            ..name = fileName
