...
class BackupService extends ChangeNotifier {
  ...
  Future<bool> backupDatabase() async {
    try {
      ...
      // Upload file using stream to save memory and start faster
      ...
      await _saveBackupInfo(backupFileName, uploadedFile.id!, await dbFile.length());
  ...
  Future<bool> backupImages(List<String> imagePaths, {String projectName = 'Mặc định'}) async {
    debugPrint('🚀 Backup Service Version 2.0 - Đang backup ${imagePaths.length} ảnh cho dự án $projectName');
    try {
      if (_driveApi == null) {
        final signedIn = await signInSilently();
        if (!signedIn) return false;
      ...
      // Get list of existing files in project folder to update instead of skipping
      final existingFilesQuery = "'$projectFolderId' in parents and trashed = false";
      final existingFilesList = await _driveApi!.files.list(q: existingFilesQuery, $fields: 'files(id, name, size)');
      final existingFilesMap = {for (var f in existingFilesList.files ?? []) f.name: f};
      
      debugPrint('🔍 Drive: Found ${existingFilesMap.length} files in project folder "$projectName"');

      // Prepare batches for parallel upload
      ...
      for (final imagePath in imagePaths) {
        ...
        if (!await imageFile.exists()) {
          debugPrint('❌ Drive: Local file not found: $imagePath');
          continue;
        }

        final fileName = path.basename(imagePath);
        final existingFile = existingFilesMap[fileName];
        
        if (existingFile != null) {
          final localSize = await imageFile.length();
          final driveSize = int.tryParse(existingFile.size ?? '0') ?? 0;
          
          if (localSize == driveSize) {
            debugPrint('⏩ Drive: Skipping $fileName (same size: $localSize bytes)');
            continue;
          }
        }
        
        toUpload.add(imagePath);
      }

      debugPrint('📤 Drive: Starting parallel upload for ${toUpload.length} images (Batch size: $batchSize)');
      
      for (int i = 0; i < toUpload.length; i += batchSize) {
        final batch = toUpload.sublist(i, i + batchSize > toUpload.length ? toUpload.length : i + batchSize);
        
        await Future.wait(batch.map((imagePath) async {
          final imageFile = File(imagePath);
          final fileName = path.basename(imagePath);
          final existingFile = existingFilesMap[fileName];
          ...
          try {
            if (existingFile != null) {
              ...
              debugPrint('🔄 Drive: Updating $fileName (ID: ${existingFile.id})');
              ...
              // Create new file
              debugPrint('📤 Drive: Creating $fileName');
              final fileMetadata = drive.File()
                ..name = fileName
            ...
            debugPrint('✅ Drive: Upload successful for $fileName');
          } catch (e) {
            debugPrint('❌ Drive: Error uploading $fileName: $e');
          }
        }));
      }
  ...
  /// Tải ảnh từ Drive nếu file cục bộ bị mất
  Future<String?> downloadImageIfMissing(String oldLocalPath, {String projectName = 'Mặc định'}) async {
    try {
      final fileName = path.basename(oldLocalPath);
      ...
      final projectFolderId = await _getOrCreateSubfolder(imagesFolderId, projectName);
      if (projectFolderId == null) return null;

      // Tìm file trên Drive theo tên
      final query = "name = '$fileName' and '$projectFolderId' in parents and trashed = false";
      final list = await _driveApi!.files.list(q: query);
      
      if (list.files == null || list.files!.isEmpty) {
        debugPrint('Image $fileName not found on Drive in project $projectName');
        return null;
      }

      final driveFile = list.files!.first;
      final media = await _driveApi!.files.get(driveFile.id!, downloadOptions: drive.DownloadOptions.fullMedia) as drive.Media;
      ...
      // Lưu vào thư mục documents hiện tại
      final appDir = await getApplicationDocumentsDirectory();
      final newLocalPath = path.join(appDir.path, fileName);
      final localFile = File(newLocalPath);
      await localFile.writeAsBytes(bytes);

      debugPrint('Successfully restored image from Drive: $newLocalPath');
      return newLocalPath;
    } catch (e) {
      debugPrint('Error downloading missing image: $e');
      return null;
    }
  }

  /// Tải hàng loạt ảnh từ Drive
  Future<Map<String, String>> downloadMultipleImages(List<String> fileNames, {String projectName = 'Mặc định'}) async {
    ...
    try {
      ...
      final folderId = await getOrCreateAppFolder();
      if (folderId == null) return results;

      final imagesFolderId = await _getOrCreateImagesFolder(folderId);
      if (imagesFolderId == null) return results;

      final projectFolderId = await _getOrCreateSubfolder(imagesFolderId, projectName);
      if (projectFolderId == null) return results;

      // Để tối ưu, chúng ta sẽ lấy danh sách file trong thư mục dự án trước
      final query = "'$projectFolderId' in parents and trashed = false";
      final list = await _driveApi!.files.list(q: query, $fields: 'files(id, name)');
      
      if (list.files == null || list.files!.isEmpty) return results;

      final driveFilesMap = {for (var f in list.files!) f.name: f.id};
      final appDir = await getApplicationDocumentsDirectory();

      for (final fileName in fileNames) {
        final driveFileId = driveFilesMap[fileName];
        if (driveFileId != null) {
          try {
            final media = await _driveApi!.files.get(driveFileId, downloadOptions: drive.DownloadOptions.fullMedia) as drive.Media;
            final bytes = <int>[];
            await for (final chunk in media.stream) {
              bytes.addAll(chunk);
            }
            ...
            final newLocalPath = path.join(appDir.path, fileName);
            await File(newLocalPath).writeAsBytes(bytes);
            results[fileName] = newLocalPath;
            debugPrint('Bulk restored: $fileName');
          } catch (e) {
      ...
      debugPrint('Error in downloadMultipleImages: $e');
    }
    return results;
  }
