...
class ExcelService {
  ...
  static Future<String?> exportOvertimeForAccounting({
    ...
    try {
      debugPrint('Starting Overtime Excel export for month: $month/$year');
      
      // Chạy việc tạo Excel trong Isolate để không treo UI
      final List<int>? fileBytes = await compute(_generateOvertimeBytes, {
        'month': month,
        'year': year,
      ...
      if (fileBytes == null) {
        debugPrint('Overtime Excel bytes generation failed');
        return null;
      }
      ...
      debugPrint('Overtime Excel file saved to: $filePath');
      return filePath;
    } catch (e) {
      debugPrint('Error exporting overtime: $e');
      return null;
    }
  }

  /// Hàm chạy trong Isolate để tạo bytes cho file Overtime Excel
  static List<int>? _generateOvertimeBytes(Map<String, dynamic> params) {
    ...
    final List<Map<String, dynamic>> entriesData = params['entries'];
    
    // Chuyển đổi và lọc dữ liệu NGAY TRONG ISOLATE
    final allEntries = entriesData.map((d) => OvertimeEntry.fromMap(d)).toList();
  ...
  static void showFileActionSheet(BuildContext context, String filePath, String fileName) {
  ...
  /// Helper để ghi một dòng dữ liệu vào Excel
  ...
  static Future<String?> exportCashFlowForAccounting({
    required BuildContext context,
    ...
    try {
      debugPrint('Starting Excel export for project: $project');
      
      // Chạy toàn bộ việc lọc và tạo Excel trong Isolate để không treo UI
      final List<int>? fileBytes = await compute(_generateCashFlowBytes, {
        'project': project,
        'month': month,
        'year': year,
        'employeeName': employeeName,
      ...
      if (fileBytes == null) {
        debugPrint('Excel bytes generation failed');
        return null;
      }
      ...
      debugPrint('Error exporting cash flow: $e');
      return null;
    }
  }

  /// Hàm chạy trong Isolate để tạo bytes cho file CashFlow Excel
  static List<int>? _generateCashFlowBytes(Map<String, dynamic> params) {
    final String project = params['project'];
    final int month = params['month'];
    final int year = params['year'];
    final String employeeName = params['employeeName'];
    final List<Map<String, dynamic>> transactionsData = params['transactions'];
    
    // Chuyển đổi và lọc dữ liệu NGAY TRONG ISOLATE
    final allTransactions = transactionsData.map((d) => CashTransaction.fromMap(d)).toList();
    ...
    final totalIncome = allTransactions.where((t) => 
      t.project == project && 
      t.date.month == month && 
      t.date.year == year &&
    ...
    // Nhóm theo nhà cung cấp để gộp hàng
    filteredTransactions.sort((a, b) => (a.note ?? '').compareTo(b.note ?? ''));

    int providerStartRow = currentRow;
    String? currentProvider = filteredTransactions.isNotEmpty ? filteredTransactions[0].note : null;
    ...
    for (int i = 0; i < filteredTransactions.length; i++) {
      ...
      // Kiểm tra để gộp hàng Nhà cung cấp
      if (i > 0 && t.note != currentProvider) {
        // Gộp các hàng trước đó của nhà cung cấp cũ
        if (currentRow - 1 > providerStartRow) {
          sheet.getRangeByIndex(providerStartRow, 9, currentRow - 1, 9).merge();
        }
        // Cập nhật nhà cung cấp mới
        currentProvider = t.note;
        providerStartRow = currentRow;
      }
      ...
      if (t.imagePath != null && File(t.imagePath!).existsSync()) {
        // Sửa lỗi link: dùng dấu nháy đơn cho tên sheet có khoảng trắng/dấu
        ...
        // Giữ nguyên tỷ lệ ảnh gốc (ảnh điện thoại thường là dọc 9:16)
        // Chỉ đặt lastRow, để Syncfusion tự động tính lastColumn theo tỷ lệ
        picture.lastRow = imageRow + 35; // Chiều cao ảnh (số hàng)
        picture.lastColumn = 2; // Chiều rộng nhỏ hơn để giữ tỷ lệ dọc
        ...
        imageRow += 38; // Khoảng cách giữa các ảnh (tăng để không đè nhau)
      } else {
        sheet.getRangeByIndex(currentRow, 10).setText('Hoá đơn giấy');
    ...
    // Gộp hàng cho nhà cung cấp cuối cùng
    if (currentRow - 1 > providerStartRow) {
      sheet.getRangeByIndex(providerStartRow, 9, currentRow - 1, 9).merge();
    }
