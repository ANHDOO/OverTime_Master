...
class UpdateService extends ChangeNotifier {
  ...
  Future<void> init() async {
    ...
    // Chỉ dọn dẹp cache nếu không đang trong quá trình tải hoặc sẵn sàng cài đặt
    if (_status == DownloadStatus.idle || _status == DownloadStatus.error) {
  ...
  /// Tải changelog cho phiên bản hiện tại từ GitHub
  Future<void> loadChangelogForVersion(String version) async {
    try {
      final changelogUrl = 'https://raw.githubusercontent.com/ANHDOO/OverTime_Updates/main/release_notes/$version.md?t=${DateTime.now().millisecondsSinceEpoch}';
      final response = await http.get(Uri.parse(changelogUrl));
      if (response.statusCode == 200) {
        _currentChangelog = utf8.decode(response.bodyBytes);
        notifyListeners();
      } else {
        debugPrint('Changelog not found for version $version');
      }
    } catch (e) {
  ...
  Future<UpdateCheckResult> checkForUpdate() async {
    ...
    try {
      ...
      if (resultHasUpdate) {
        ...
        NotificationService().showUpdateAvailableNotification(remoteVersionName);
      } else {
        _updateInfo = null;
    ...
    try {
      ...
      for (var url in downloadUrls) {
        try {
          ...
          await _dio.download(
            ...
            onReceiveProgress: (received, total) {
              if (total != -1) {
                ...
                final intPercent = (_progress * 100).toInt();
                
                // Cập nhật notification mỗi 5% để tránh spam
                if (intPercent % 5 == 0 && intPercent != lastNotifiedProgress) {
                  lastNotifiedProgress = intPercent;
            ...
            options: Options(
              ...
              receiveTimeout: const Duration(minutes: 30), // Tăng lên 30 phút
          ...
          if (await downloadedFile.exists() && await downloadedFile.length() > 1024 * 1024) {
            success = true;
            debugPrint('✅ Tải thành công: ${await downloadedFile.length()} bytes');
            break;
          } else {
            lastError = 'File tải về không hợp lệ';
          }
        } catch (e) {
          debugPrint('⚠️ Lỗi với nguồn $url: $e');
          lastError = e.toString();
          // Đợi một chút trước khi thử nguồn khác (hữu ích khi app vừa xuống background)
          await Future.delayed(const Duration(seconds: 2));
        }
      }

      // Xóa notification tiến trình
      ...
      if (success) {
        ...
        // Thông báo tải xong
        NotificationService().showUpdateDownloadedNotification(_updateInfo!.versionName);
      } else {
        _status = DownloadStatus.error;
        _error = 'Không thể tải bản cập nhật. Vui lòng thử lại sau.';
        // Thông báo lỗi tải
        NotificationService().showUpdateErrorNotification(_error!);
      }
      notifyListeners();
    } catch (e) {
      _status = DownloadStatus.error;
