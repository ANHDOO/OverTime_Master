...
class _StatisticsScreenState extends State<StatisticsScreen> with TickerProviderStateMixin {
  ...
  void _showExportDialog(BuildContext parentContext) {
    final provider = Provider.of<OvertimeProvider>(parentContext, listen: false);
                  ...
                  return Container(
                    ...
                    child: ListTile(
                      ...
                      onTap: () async {
                        ...
                        // Đợi một chút để bottom sheet đóng hẳn
                        ...
                        try {
                          debugPrint('Initiating Overtime Excel export...');
                          // Thêm delay để UI kịp vẽ Loading Dialog
                          await Future.delayed(const Duration(milliseconds: 500));
                          
                          final filePath = await ExcelService.exportOvertimeForAccounting(
                          ...
                          debugPrint('Excel export completed. filePath: $filePath, mounted: $mounted');
                          
                          if (mounted) {
                            Navigator.of(parentContext, rootNavigator: true).pop(); // Đóng loading
                            debugPrint('Loading dialog closed');
                            if (filePath != null) {
                              ExcelService.showFileActionSheet(parentContext, filePath, 'TangCa_T${month.month}_${month.year}.xlsx');
                            } else {
                              ScaffoldMessenger.of(parentContext).showSnackBar(
                                const SnackBar(content: Text('Không có dữ liệu tăng ca trong tháng này'), backgroundColor: Colors.orange),
                              );
                            }
                          }
                        } catch (e) {
  ...
  void _showCashFlowExportDialog(BuildContext parentContext) {
    final provider = Provider.of<CashTransactionProvider>(parentContext, listen: false);
    final isDark = Theme.of(parentContext).brightness == Brightness.dark;
              ...
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  onPressed: () async {
                    ...
                    try {
                      debugPrint('Initiating CashFlow Excel export...');
                      // Thêm delay để UI kịp vẽ Loading Dialog
                      await Future.delayed(const Duration(milliseconds: 500));
                      
                      final filePath = await ExcelService.exportCashFlowForAccounting(
                        context: parentContext,
                      ...
                      debugPrint('CashFlow Excel export completed. filePath: $filePath, mounted: $mounted');
                      
                      if (mounted) {
                        Navigator.of(parentContext, rootNavigator: true).pop(); // Đóng loading
                        debugPrint('Loading dialog closed');
                        if (filePath != null) {
                          ExcelService.showFileActionSheet(parentContext, filePath, 'PhieuGiaiChi_${selectedProject}_T${selectedMonth.month}_${selectedMonth.year}.xlsx');
                        } else {
                          ScaffoldMessenger.of(parentContext).showSnackBar(
                            const SnackBar(content: Text('Không có dữ liệu chi ra để xuất'), backgroundColor: Colors.orange),
                          );
                        }
                      }
                    } catch (e) {
                      debugPrint('CashFlow Excel export error: $e');
                      if (mounted) {
                        Navigator.of(parentContext, rootNavigator: true).pop();
                        ScaffoldMessenger.of(parentContext).showSnackBar(
                          SnackBar(content: Text('Lỗi: $e'), backgroundColor: Colors.red),
                  ...
                  if (value.toInt() < otData.length) {
                    return Padding(
                      ...
                      child: Text(
                        ...
                        style: TextStyle(fontSize: 10, color: isDark ? AppColors.darkTextSecondary : AppColors.lightTextSecondary, fontWeight: FontWeight.w700),
                      ),
                    );
                  }
                  return const Text('');
            ...
            leftTitles: AxisTitles(
              sideTitles: SideTitles(
                ...
                getTitlesWidget: (value, meta) {
                  ...
                  return Text('${(value / 1000000).toStringAsFixed(0)}M', style: TextStyle(fontSize: 10, color: isDark ? AppColors.darkTextSecondary : AppColors.lightTextSecondary, fontWeight: FontWeight.w600));
                },
                reservedSize: 35,
              ),
            ),
                          ...
                          Positioned.fill(
                            child: Center(
                              child: Text(
                                ...
                                style: TextStyle(fontSize: 10, fontWeight: FontWeight.w800, color: barWidth > 0.4 ? Colors.white : AppColors.accentDark),
                              ),
                            ),
                          ),
                        ],
              ...
              sideTitles: SideTitles(
                ...
                getTitlesWidget: (value, meta) {
                  if (value.toInt() < cashData.length) {
                    return Padding(padding: const EdgeInsets.only(top: 8), child: Text(DateFormat('MM/yy').format(cashData[value.toInt()]['month']), style: TextStyle(fontSize: 10, color: isDark ? AppColors.darkTextSecondary : AppColors.lightTextSecondary, fontWeight: FontWeight.w700)));
                  }
                  return const Text('');
                },
              ),
            ),
            leftTitles: AxisTitles(sideTitles: SideTitles(showTitles: true, interval: 5000000, getTitlesWidget: (value, meta) { if (value == 0) return const Text(''); return Text('${(value / 1000000).toStringAsFixed(0)}M', style: TextStyle(fontSize: 10, color: isDark ? AppColors.darkTextSecondary : AppColors.lightTextSecondary, fontWeight: FontWeight.w600)); }, reservedSize: 35)),
          ),
          borderData: FlBorderData(show: false),
          lineBarsData: [
      ...
      children: [
        Expanded(
          child: Container(
            padding: const EdgeInsets.all(16),
            decoration: BoxDecoration(
              gradient: LinearGradient(colors: [AppColors.primary.withOpacity(isDark ? 0.25 : 0.15), AppColors.primaryDark.withOpacity(isDark ? 0.2 : 0.1)]),
              borderRadius: AppRadius.borderLg,
        ...
        Expanded(
          child: Container(
            padding: const EdgeInsets.all(16),
            decoration: BoxDecoration(
              gradient: LinearGradient(colors: [AppColors.success.withOpacity(isDark ? 0.25 : 0.15), AppColors.successDark.withOpacity(isDark ? 0.2 : 0.1)]),
              borderRadius: AppRadius.borderLg,
