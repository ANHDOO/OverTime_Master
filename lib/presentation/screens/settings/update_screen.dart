...
class _UpdateScreenState extends State<UpdateScreen> {
  ...
  void initState() {
    super.initState();
    // Chỉ init nếu không đang tải
    if (_updateService.status != DownloadStatus.downloading) {
      _updateService.init();
    }
    _calculateStorage();
    _loadCurrentChangelog();
  }

  Future<void> _loadCurrentChangelog() async {
    // Tải changelog của version hiện tại từ release_notes
    final packageInfo = await PackageInfo.fromPlatform();
    _updateService.loadChangelogForVersion(packageInfo.version);
  }
        ...
        SnackBar(
          content: Row(
            children: [
              ...
              const Text('Đã dọn dẹp bộ nhớ đệm thành công'),
            ],
          ),
    ...
    return Column(
      ...
      children: [
        ...
        Container(
          ...
          child: Column(
            children: [
              _buildStorageItem('File APK cập nhật', _cacheSize, Icons.system_update_alt_rounded, isDark),
              const SizedBox(height: 16),
              SizedBox(
                width: double.infinity,
                child: Container(
                  decoration: BoxDecoration(
                    gradient: _cacheSize > 0.1 ? AppGradients.heroOrange : null,
                    borderRadius: AppRadius.borderMd,
                  ),
                  child: ElevatedButton.icon(
                    onPressed: _cacheSize > 0.1 ? _cleanup : null,
                    icon: const Icon(Icons.cleaning_services_rounded, size: 18),
                    label: Text(_cacheSize > 0.1 ? 'Xóa cache (${_cacheSize.toStringAsFixed(1)} MB)' : 'Không có cache'),
                    style: ElevatedButton.styleFrom(
                      backgroundColor: _cacheSize > 0.1 ? Colors.transparent : (isDark ? AppColors.darkSurfaceVariant : AppColors.lightSurfaceVariant),
                      foregroundColor: _cacheSize > 0.1 ? Colors.white : (isDark ? AppColors.darkTextMuted : AppColors.lightTextMuted),
  ...
  Widget _buildFeaturesSection(bool isDark) {
    return Column(
      ...
      children: [
        Text(
          _updateService.hasUpdate || _updateService.status == DownloadStatus.downloading
              ? 'Nhật ký phiên bản mới'
        ...
        Container(
          ...
          child: (_updateService.hasUpdate || _updateService.status == DownloadStatus.downloading)
              ? _buildChangelogContent(_updateService.newChangelog ?? _updateService.updateInfo?.changelog ?? '', isDark)
              : (_updateService.currentChangelog != null && _updateService.currentChangelog!.isNotEmpty
  ...
  Widget _buildChangelogContent(String changelog, bool isDark) {
    ...
    for (var line in lines) {
      ...
      if (line.startsWith('# ')) {
        ...
        widgets.add(Padding(
          padding: EdgeInsets.only(left: 8.0 + (indent > 0 ? indent * 2 : 0), top: 2),
          child: Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              ...
              Expanded(
                child: Text(
                  trimmedLine.substring(2).replaceAll('**', ''),
