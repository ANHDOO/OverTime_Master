    ...
    try {
      ...
      if (results['database'] == true) {
        try {
          ...
          // Tự động khôi phục ảnh từ Google Drive (chạy ngầm)
          debugPrint('Starting auto restore images from Drive...');
          Provider.of<CashTransactionProvider>(context, listen: false).autoRestoreMissingImages();
        } catch (e) {
        ...
        await _loadBackupData();
        if (!mounted) return;
        _showSuccess('Khôi phục thành công! Đang tải ảnh từ Drive...');
      } else {
