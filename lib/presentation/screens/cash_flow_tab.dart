...
class _CashFlowTabState extends State<CashFlowTab> {
  ...
  /// Lấy danh sách đường dẫn ảnh từ các giao dịch
  List<String> _getImagePaths(List<CashTransaction> transactions) {
    return transactions
  ...
  /// Hiển thị dialog chọn và in ảnh chứng từ
  void _printReceiptsPdf(BuildContext context, List<CashTransaction> transactions, bool isDark) {
    final imagePaths = _getImagePaths(transactions);
    
    if (imagePaths.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Không có ảnh chứng từ để in')),
      );
      return;
    }
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                children: [
                  Container(
                    ...
                    child: Icon(Icons.print_rounded, color: AppColors.primary),
                  ),
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          '${imagePaths.length} ảnh • ${(imagePaths.length / 9).ceil()} trang A4',
                          style: TextStyle(
                            fontSize: 13,
              ...
              Text(
                'Layout: 9 ảnh/trang, tự động sắp xếp cân đối',
                style: TextStyle(
                  fontSize: 13,
                  ...
                  Expanded(
                    child: OutlinedButton.icon(
                      onPressed: () {
                        ...
                        ReceiptPdfService.saveAndShare(
                          context,
                          imagePaths,
                          fileName: 'ChungTu_${_selectedProject == 'Tất cả' ? 'TatCa' : _selectedProject}_${DateTime.now().millisecondsSinceEpoch}.pdf',
                        );
                      },
                  ...
                  Expanded(
                    ...
                    child: Container(
                      ...
                      child: ElevatedButton.icon(
                        onPressed: () {
                          Navigator.pop(context);
                          ReceiptPdfService.previewAndPrint(
                            context,
                            imagePaths,
                            title: 'ChungTu_${_selectedProject == 'Tất cả' ? 'TatCa' : _selectedProject}',
                          );
                        },
                        icon: const Icon(Icons.print_rounded, color: Colors.white),
                        label: const Text('Xem & In', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700)),
              ...
              child: Row(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      // Nút in ảnh chứng từ
                      if (_getImagePaths(filteredTransactions).isNotEmpty)
                        Container(
                          ...
                          child: IconButton(
                            ...
                            tooltip: 'In ảnh chứng từ',
                            onPressed: () => _printReceiptsPdf(context, filteredTransactions, isDark),
