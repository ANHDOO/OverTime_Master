...
class GoldHighlightCard extends StatelessWidget {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: isUp 
            ...
            : (provider.priceChangePercent < 0 ? AppColors.danger.withOpacity(0.9) : Colors.black.withOpacity(0.2)),
  ...
  Widget _buildChart(GoldProvider provider) {
    if (provider.priceHistory.length < 2) {
      return SizedBox(
        height: 140,
        child: Center(child: Text('Đang thu thập dữ liệu biểu đồ...', style: TextStyle(color: Colors.white.withOpacity(0.7), fontSize: 12))),
      );
    }
                  ...
                  if (value.toInt() >= 0 && value.toInt() < provider.priceHistory.length) {
                    ...
                    try {
                      ...
                      return Padding(
                        ...
                        child: Text(DateFormat('dd/MM').format(date), style: TextStyle(color: Colors.white.withOpacity(0.8), fontSize: 10, fontWeight: FontWeight.w600)),
                      );
                    } catch (_) {
                      return Padding(
                        padding: const EdgeInsets.only(top: 8.0),
  ...
  Widget _buildPriceInfo(String label, double price, Color color) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(label, style: TextStyle(color: color.withOpacity(0.8), fontSize: 11, fontWeight: FontWeight.w800, letterSpacing: 1.5)),
