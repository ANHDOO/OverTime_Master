...
class _MstSearchScreenState extends State<MstSearchScreen> {
  ...
  Future<void> _extractCaptcha() async {
    try {
      if (!mounted) return;
      // Không cập nhật status nếu đang hiển thị kết quả hoặc đang load kết quả
      ...
      if (!captchaReady) throw 'Captcha không tải được';

      final captchaData = await _headlessController!.runJavaScriptReturningResult('''
        (function() { 
          var img = document.querySelector('img[src*="captcha"]'); 
          if (!img || !img.complete || img.naturalWidth === 0) return JSON.stringify({src: '', base64: ''}); 
          var canvas = document.createElement('canvas'); 
          canvas.width = img.naturalWidth; 
          canvas.height = img.naturalHeight; 
          var ctx = canvas.getContext('2d'); 
          ctx.drawImage(img, 0, 0); 
          return JSON.stringify({src: img.src, base64: canvas.toDataURL('image/png').split(',')[1]}); 
        })()
      ''');
      ...
      if (decodedData is Map) {
        final String base64Str = decodedData['base64'] ?? '';
        if (mounted && base64Str.isNotEmpty && base64Str.length > 100) {
          final bytes = base64Decode(base64Str);
          setState(() { 
            _captchaBytes = bytes; 
            _isLoadingCaptcha = false; 
            if (!_showResults && !_isLoading) _statusMessage = 'Vui lòng nhập mã xác thực'; 
          });
          return;
        }
      }
      ...
      else if (mounted) setState(() { _isLoadingCaptcha = false; });
  ...
  Future<void> _performSearch() async {
    if (_mstController.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Row(children: [const Icon(Icons.error_rounded, color: Colors.white), const SizedBox(width: 10), const Text('Vui lòng nhập MST')]), backgroundColor: AppColors.danger, behavior: SnackBarBehavior.floating, shape: RoundedRectangleBorder(borderRadius: AppRadius.borderMd)));
      return;
    }
    ...
    setState(() { _isLoading = true; _statusMessage = 'Đang gửi yêu cầu...'; _errorMessage = null; _showResults = false; });
    _saveLastSearch();
    ...
    try {
      ...
      // Chờ trang load và kiểm tra kết quả
      bool foundResult = false;
      for (int i = 0; i < 15; i++) {
        await Future.delayed(const Duration(seconds: 1));
        if (!mounted) return;
        
        final pageCheck = await _headlessController!.runJavaScriptReturningResult('''(function() { 
          var bodyText = document.body.innerText;
          if (bodyText.includes('Vui lòng nhập đúng mã xác nhận')) return 'CAPTCHA_ERROR'; 
          if (bodyText.includes('BẢNG THÔNG TIN TRA CỨU')) return 'HAS_DATA'; 
          if (bodyText.includes('Không tìm thấy')) return 'NO_DATA'; 
          return 'WAITING'; 
        })()''');
        
        String status = pageCheck.toString().replaceAll('"', '');
        debugPrint('[MST] Search status: $status (attempt $i)');
        
        if (status == 'CAPTCHA_ERROR') {
          setState(() { _errorMessage = 'Mã xác thực không chính xác. Vui lòng thử lại.'; _isLoading = false; });
          _initHeadlessWebView();
          return;
        }
      ...
      if (!foundResult) {
        setState(() { _errorMessage = 'Hệ thống phản hồi chậm, vui lòng thử lại.'; _isLoading = false; });
        return;
      }
      ...
      final result = await _headlessController!.runJavaScriptReturningResult('''
        (function() { 
          ...
          var table = tables.find(t => t.innerText.includes('Tên người nộp thuế') && t.innerText.includes('Cơ quan thuế'));
          ...
          for(var i = 0; i < rows.length; i++) {
            ...
            if(cells.length >= 5) {
              var mst = cells[1]?.innerText?.trim() || '';
              if(/^[0-9]{10,}/.test(mst)) {
                data.push({
                  'mst': mst,
                  'name': cells[2]?.innerText?.trim() || '',
                  'agency': cells[3]?.innerText?.trim() || '',
                  'status': cells[4]?.innerText?.trim() || 'Đang hoạt động'
                });
              }
            }
          }
      ...
      if (mounted) {
        setState(() { 
          ...
          _showResults = true; 
          _isLoading = false; 
          _statusMessage = _results.isEmpty ? 'Không tìm thấy thông tin' : 'Tìm thấy ${_results.length} kết quả'; 
        });
      }
    } catch (e) {
  ...
  void _useProfile(CitizenProfile profile) {
    setState(() { _mstController.text = profile.taxId ?? ''; });
