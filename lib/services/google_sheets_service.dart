...
class GoogleSheetsService {
  ...
  Future<bool> syncProjectDetails({
    ...
    try {
      ...
      // A. Clear cột A, B & C (Expenses + Notes) từ hàng 2 trở đi để xóa dữ liệu cũ
      requests.add({
        'updateCells': {
          'range': {
            ...
            'endColumnIndex': 3, // Col D (exclusive) -> A, B & C
          },
          'fields': 'userEnteredValue' // Chỉ clear giá trị
        }
      });

      // B. Ghi danh sách chi tiêu mới vào cột A, B & C (bắt đầu từ A2)
      if (expenses.isNotEmpty) {
        ...
        // Thêm border cho vùng Expenses (A1:C{n})
      ...
      // D. Ghi Header cho cột Expenses (A1-C1) nếu cần
      requests.add({
        'updateCells': {
          'start': {
            'sheetId': int.parse(sheetId),
      ...
      // Thêm border cho vùng Summary (D1:E4)
      requests.add({
        'updateBorders': {
          'range': {
            'sheetId': int.parse(sheetId),
